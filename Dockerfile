...
RUN apt-get install -y php8.1-gd php8.1-bz2 php8.1-zip php8.1-curl php8.1-gd php8.1-mysql php8.1-xml php8.1-dev php8.1-sqlite php8.1-mbstring php8.1-bcmath

# Install PHP 8.1 Intl
RUN apt-get update
...
# Install PHP Sql Server
RUN curl https://packages.microsoft.com/keys/microsoft.asc | apt-key add - && \
    curl https://packages.microsoft.com/config/ubuntu/20.04/prod.list > /etc/apt/sources.list.d/mssql-release.list && \
    apt-get -y update && \
    export ACCEPT_EULA=Y && apt-get -y install msodbcsql17 mssql-tools unixodbc-dev
...
RUN sed -i -e 's/^zpost_max_size\s*=.*/post_max_size = 32M/' /etc/php/8.1/apache2/php.ini
RUN sed -i -e 's/^upload_max_filesize\s*=.*/upload_max_filesize = 32M/' /etc/php/8.1/apache2/php.ini
RUN sed -i -e 's/^max_execution_time\s*=.*/max_execution_time = 300/' /etc/php/8.1/apache2/php.ini
