	...
	function z(e) {
		if (e == e) {
			...
			switch (e["data-layout-mode"]) {
				case "light":
					x("data-layout-mode", "light"),
						document.documentElement.setAttribute("data-layout-mode", "light"),
						sessionStorage.setItem("data-layout-mode", "light");
					break;
				case "dark":
					x("data-layout-mode", "dark"),
		...
		document
			...
			.forEach(function (t) {
				t.addEventListener("click", function (e) {
					...
					// Hapus kelas mode-auto jika ada
					document.documentElement.classList.remove("mode-auto");

					// Sesuaikan tema berdasarkan opsi yang dipilih
					if (t.getAttribute("data-mode") === "light") {
						// Set tema light
						l.setAttribute("data-layout-mode", "light");
						sessionStorage.setItem("data-layout-mode", "light");
						sessionStorage.setItem("data-layout-auto", "false");
					} else if (t.getAttribute("data-mode") === "dark") {
						// Set tema dark
						l.setAttribute("data-layout-mode", "dark");
						sessionStorage.setItem("data-layout-mode", "dark");
						sessionStorage.setItem("data-layout-auto", "false");
					} else if (t.getAttribute("data-mode") === "auto") {
						// Set tema auto (sesuai sistem)
...
mybutton &&
	...
	setInterval(() => {
		...
		// Format tanggal: Minggu, 3 Agustus 2025
		const date = e.toLocaleDateString("id-ID", {
			weekday: "long",
			day: "numeric",
			month: "long",
			year: "numeric",
		...
		// Format waktu: 13:45
		const time =
			("0" + e.getHours()).slice(-2) + ":" + ("0" + e.getMinutes()).slice(-2);

		// Gabungkan
		const result = `${date} ${time}`;

		document.getElementById("current-time").innerHTML = result;
	}, 1e3);

// Fungsi untuk memperbarui ikon tema berdasarkan tema yang aktif
function updateThemeIcon() {
	const themeButton = document.querySelector(".mode-layout i");
	const currentTheme =
...
// Inisialisasi ikon tema saat halaman dimuat
document.addEventListener("DOMContentLoaded", function () {
	updateThemeIcon();
});

// Jika tema auto, tambahkan listener untuk perubahan preferensi sistem
if (sessionStorage.getItem("data-layout-auto") === "true") {
	window
